...
error_reporting(E_ALL ^ E_NOTICE);
	...
	if(@$_FILES['uploadedfile']['name']!='')
	{
		...
		while ($line = fgets($fh)) 
		{
			...
			if($result = $db->get_results("select cps_unique_req from tb_print_req_collection where cps_account_no = '$explode[6]' and cps_chq_no_from = '$explode[23]' "))
			{
				echo "<p style='color:red'>This file has records which are already printed</p>";
				$entries_found = true;
				break;
			}
			else if($result = $db->get_results("select cps_unique_req from tb_uploadingdata where cps_account_no = '$explode[6]' and cps_chq_no_from = '$explode[23]' "))
			{
		...
		if(!$entries_found)
		{
			...
			foreach ($data as $key => $explode) {
			 ...
			 $sql="insert into tb_uploadingdata(
				...
				cps_act_mobile, cps_act_telephone_res, cps_act_telephone_off,branch_sub_code,cps_atpar

				)values(
				'$explode[0]','$cpsmicr_code','$cpsbranch_code','$explode[7]', '$explode[6]',
				'$name',
				'$explode[9]','$explode[10]','$addresses1','$addresses2','$addresses3',
				'$addresses4','$addresses5','$explode[17]','$explode[18]','$explode[19]', '$explode[20]',
				'Y', '$explode[23]','$explode[24]',

				'$dates','$dates', '$explode[5]', '$cpsprocess_user_id', '$explode[36]', 
				'$explode[11]', '$explode[12]', '$explode[13]',
				'$explode[25]', '$explode[26]', '$explode[27]','$explode[3]','$exploade[22]'
				)";
